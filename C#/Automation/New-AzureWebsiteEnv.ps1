...
Function New-PublishXml
{
    ...
    $website = Get-AzWebApp -Name $WebsiteName -ResourceGroupName $resourceGroupName
	...
	echo "Get the publish settings info from the REST API"
    $publishSettings = Invoke-RestMethod -Method POST -Uri $uri -Certificate $cert -Headers $Headers
...
$sqlAppDatabaseServer = $Name + "server"
$sqlAppDatabaseName = "appdb"
$sqlMemberDatabaseName = "memberdb"
$sqlDatabaseServerFirewallRuleName = $Name + "rule"

Write-Verbose "Creating a Windows Azure Resource Group: $resourceGroupName"
...
if (!$resourcegroup) {throw "Error: Resource Group was not created. Terminating the script unsuccessfully. Fix the errors that New-AzureWebsite returned and try again."}

Write-Verbose "Creating a Windows Azure website: $Name"
# Create a new website
$website = New-AzResource -ResourceType microsoft.web/sites -Name $Name -Location $Location -ResourceGroupName $resourceGroupName -Properties @{test="test"} -Force -Verbose
...
$thumbprint = Get-Content -LiteralPath "thumbprint.txt"
...
$uri = "https://management.azure.com/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Web/sites/{2}/publishxml?api-version=2019-08-01" -f `
         $s.SubscriptionId, $resourceGroupName, $Website.Name

$principal = az ad sp create-for-rbac -n "PublishFixIt"
$namepublish = ($principal| ConvertFrom-Json).name
$password = ($principal| ConvertFrom-Json).password
$Body = @{
    grant_type  = "client_credentials"
    client_id   = $namepublish
    client_secret      = $password
    resource     = "https://management.azure.com"}
$tenantId = $s.TenantId
$AuthUrl = "https://login.microsoftonline.com/$tenantId/oauth2/token";

#We need to wait for a few seconds for the secrets to be ?syncronized? before calling the Rest Method
#Start-Sleep 10

Write-Verbose "Creating a Windows Azure storage account: $storageAccountName"
# Create a new storage account
$storage = & "$scriptPath\New-AzureStorage.ps1" -Name $storageAccountName -Location $Location -ResourceGroupName $resourceGroupName
...
$sql = & "$scriptPath\New-AzureSql.ps1" `
    ...
    -DatabaseServerName $sqlAppDatabaseServer `
...
$storageAccessKey =(Get-AzStorageAccountKey -Name $storageAccountName -ResourceGroupName $resourceGroupName)| Where-Object {$_.KeyName -eq "key1"}
$appSettings = @{ `
    "StorageAccountName" = $storageAccountName; `
	"StorageAccountAccessKey" = $storageAccessKey.Value; `
...
$connectionStrings = @{ `
    $sqlAppDatabaseName = @{Type = "SQLAzure"; Value = $sql.AppDatabase.ConnectionString;}; `
    "DefaultConnection" = @{Type = "SQLAzure"; Value = $sql.MemberDatabase.ConnectionString;} `
}
...
$error.clear()
Set-AzWebapp -Name $Name -ResourceGroupName $resourceGroupName -AppSettings $appSettings -ConnectionStrings $connectionStrings
...
$error.clear()
Restart-AzWebapp -Name $Name -resourceGroupName $resourceGroupName
...
$responseXml = $(Invoke-RestMethod -Method POST -Uri $AuthUrl -Body $Body)
$Bearer = $responseXml.access_token

Write-Verbose "[Begin] generating $Name.pubxml file"
New-PublishXml -Website $Name -Bearer $Bearer -Thumbprint $thumbprint
